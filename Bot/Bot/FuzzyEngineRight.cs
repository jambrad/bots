...
namespace Robot
{
    class FuzzyEngineRight
    {
        ...
        private void InitializeFAM()
        {
            ...
            SPEED_FAM[(int)ANGLE.VERY_RIGHT][(int)DISTANCE.MEDIUM] = (int)SPEED.VERY_SLOW;
            SPEED_FAM[(int)ANGLE.VERY_RIGHT][(int)DISTANCE.FAR] = (int)SPEED.SLOW;
            SPEED_FAM[(int)ANGLE.VERY_RIGHT][(int)DISTANCE.VERY_FAR] = (int)SPEED.SLOW;
            ...
            SPEED_FAM[(int)ANGLE.SLIGHTLY_RIGHT][(int)DISTANCE.MEDIUM] = (int)SPEED.SLOW;
            ...
            // SLIGHTLY LEFT row
            SPEED_FAM[(int)ANGLE.SLIGHTLY_LEFT][(int)DISTANCE.VERY_NEAR] = (int)SPEED.AVERAGE;
            SPEED_FAM[(int)ANGLE.SLIGHTLY_LEFT][(int)DISTANCE.NEAR] = (int)SPEED.AVERAGE;
        ...
        public double getEngineOutput(double input_angle, double input_distance)
        {
            ...
            for (i = 0; i < NUM_ANGLE_SET; i++)
            {
                for (j = 0; j < NUM_DISTANCE_SET; j++)
                {
                    ...
                    {
                        //Console.WriteLine("RIGHTAngle: " + (ANGLE)i + "({0})" + "   Distance: " + (DISTANCE)j + "({1})", AngleSet[i].membership(input_angle), DistanceSet[j].membership(input_distance));
